...
# otherwise-correct fixes here, so `local` always rebuilds from scratch. Use
# `local-fast` while iterating, and `local` before trusting a result.
local: check setup
	@rm -rf "$(LOCAL_DERIVED_DATA)"
	@$(MAKE) local-fast

# Same build, install, and launch, but incremental. Minutes instead of a full
# rebuild; do not use it to verify a fix that "isn't taking".
local-fast: check setup
...
help:
	...
	@echo "  local              Clean build, sign, install, and launch the local app"
